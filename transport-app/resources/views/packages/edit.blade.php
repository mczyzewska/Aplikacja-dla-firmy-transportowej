...
<div class="container mx-auto p-6 max-w-5xl">
    ...
    <header class="mb-10 border-b-4 border-blue-800 pb-4">
        <h1 class="text-4xl font-black text-gray-900 uppercase tracking-tighter">
            Pełna Edycja Przesyłki: {{ $package->tracking_number }}
        </h1>
    ...
    {{-- KARTA EDYCJI (Styl: górny pas 12px - Niebieski Logistyka) --}}
    <div class="bg-white p-10 rounded-xl shadow-2xl border-2 border-gray-100 border-t-[12px] border-t-blue-800">
        <form action="{{ route('admin.packages.update', $package) }}" method="POST" class="space-y-10" novalidate>
            @csrf @method('PUT')

            {{-- SEKCJA 1: Identyfikacja i Klient --}}
            <section aria-labelledby="section-basic">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8 pb-10 border-b-4 border-gray-50">
                    {{-- Numer Trackingowy (Zasada: Powiązanie Label-Input) [cite: 278] --}}
                    <div>
                    ...
                    <div>
                        ...
                        <select id="client_id" name="client_id" required aria-required="true"
                            ...
                            @foreach($clients as $c)
                                <option value="{{ $c->id }}" {{ $package->client_id == $c->id ? 'selected' : '' }}>
                                    {{ $c->user->name ?? $c->name }}
                                </option>
                            @endforeach
                        </select>
            ...
            {{-- SEKCJA 2: Logistyka i Transport --}}
                    ...
                    <div>
                        ...
                        <select id="warehouse_id" name="warehouse_id" 
                            ...
                            @foreach($warehouses as $w)
                                <option value="{{ $w->id }}" {{ $package->warehouse_id == $w->id ? 'selected' : '' }}>
                                    {{ $w->name }} ({{ $w->city }})
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div>
                        ...
                        <select id="courier_id" name="courier_id" 
                            ...
                            @foreach($couriers as $cr)
                                <option value="{{ $cr->id }}" {{ $package->courier_id == $cr->id ? 'selected' : '' }}>
                    ...
                    <div>
                        ...
                        <select id="pickup_point_id" name="pickup_point_id" 
                            ...
                            @foreach($pickupPoints as $p)
                                <option value="{{ $p->id }}" {{ $package->pickup_point_id == $p->id ? 'selected' : '' }}>
                                    {{ $p->city }} ({{ $p->code }})
                                </option>
                            @endforeach
                        </select>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <div>
                        ...
                        <select id="status" name="status" 
                            ...
                            @foreach(['odebrana', 'w_transporcie', 'w_punkcie', 'dostarczona'] as $st)
                                <option value="{{ $st }}" {{ $package->status == $st ? 'selected' : '' }}>
                                    ● {{ str_replace('_', ' ', $st) }}
                                </option>
                            @endforeach
                        </select>
            ...
            {{-- Przyciski Akcji (Zasada: Funkcjonalność - wyraźny fokus)  --}}
