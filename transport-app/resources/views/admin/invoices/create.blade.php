...
<div class="container mx-auto p-6 max-w-5xl">
    ...
    <div class="bg-white p-6 rounded-xl shadow-xl border-2 border-gray-100 border-t-[12px] border-t-blue-800 mb-8">
        <form method="GET" action="{{ route('admin.invoices.create') }}" id="client_form" novalidate>
    ...
    <div class="bg-white p-8 rounded-xl shadow-xl border-2 border-gray-100 border-t-[12px] border-t-green-600">
        <form method="POST" action="{{ route('admin.invoices.store') }}" novalidate>
            @csrf
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
                <div>
                    ...
                    <input type="text" name="nip" id="nip" value="{{ old('nip', $selectedClient->nip) }}"
                ...
                <div>
                    ...
                    <input type="date" name="issue_date" id="issue_date" value="{{ now()->toDateString() }}" class="w-full h-12 border-2 border-gray-100 rounded-lg px-4 bg-gray-50 font-bold" required aria-required="true">
                </div>

                <div>
                    <label for="due_date" class="block text-[10px] font-black text-gray-700 uppercase mb-2 tracking-widest">Termin Płatności</label>
                    <input type="date" name="due_date" id="due_date" value="{{ now()->addDays(14)->toDateString() }}" class="w-full h-12 border-2 border-gray-100 rounded-lg px-4 bg-gray-50 font-bold" required aria-required="true">
                </div>
            </div>
                    ...
                    <tbody class="divide-y-2 divide-gray-100">
                        ...
                        <tr class="hover:bg-green-50 transition-colors">
                            <td class="p-5 text-center">
                                <input type="checkbox" name="items[{{ $index }}][package_id]" id="pkg_{{ $package->id }}" value="{{ $package->id }}" 
                                       aria-label="Dołącz paczkę {{ $package->tracking_number }} do faktury"
                            ...
                            <td class="p-5">
                                <label for="pkg_{{ $package->id }}" class="cursor-pointer">
                                    <div class="font-black text-gray-900 uppercase tracking-tight text-sm">{{ $package->tracking_number }}</div>
                                    <div class="text-[10px] text-gray-500 font-black uppercase tracking-widest mt-1">Waga: {{ $package->weight }} KG</div>
                                </label>
                            </td>
                            <td class="p-5">
                                <div class="relative">
                                    <input type="number" step="0.01" name="items[{{ $index }}][price]" 
                                           aria-label="Cena netto dla paczki {{ $package->tracking_number }}"
            ...
            <div class="flex items-center justify-between pt-8 border-t-4 border-gray-50">
                <a href="{{ route('admin.invoices.index') }}" class="text-[10px] font-black text-gray-400 uppercase tracking-[0.3em] hover:text-red-600 transition-colors">
