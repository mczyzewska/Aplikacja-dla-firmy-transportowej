...
<div class="container mx-auto p-6 max-w-7xl">
    {{-- 1. NAGŁÓWEK SYSTEMOWY (Zasada: Postrzegalność) --}}
    ...
    {{-- 2. KARTA REJESTRACJI (Fioletowy - Kadry) --}}
    ...
    {{-- 3. TABELE Z PODZIAŁEM NA ROLE (Zasada: Solidność - nagłówki scope="col") --}}
    <div class="space-y-16">
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y-2 divide-gray-50 text-sm">
                        ...
                        <tr class="hover:bg-red-50 transition-colors">
                            <td class="p-5 font-black text-gray-900 uppercase tracking-tight">{{ $u->name }}</td>
                            <td class="p-5 text-gray-600 font-mono uppercase">{{ $u->email }}</td>
                            <td class="p-5 text-center">
                                <a href="{{ route('admin.users.edit', $u) }}" aria-label="Edytuj administratora: {{ $u->name }}" class="bg-blue-800 text-white px-4 py-2 rounded font-black text-[10px] uppercase shadow-md hover:bg-black transition-all">Edycja</a>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
                        ...
                        <tr class="hover:bg-blue-50 transition-colors">
                            ...
                            <td class="p-5">
                                <div class="flex justify-center gap-3">
                                    <a href="{{ route('admin.users.edit', $u) }}" aria-label="Edytuj pracownika: {{ $u->name }}" class="bg-blue-800 text-white px-4 py-2 rounded font-black text-[10px] uppercase hover:bg-black transition-all">Edycja</a>
                                    <form action="{{ route('admin.users.destroy', $u) }}" method="POST" onsubmit="return confirm('Usunąć konto pracownika?')">
                                        @csrf @method('DELETE')
                                        <button aria-label="Usuń pracownika: {{ $u->name }}" class="bg-red-700 text-white px-4 py-2 rounded font-black text-[10px] uppercase shadow-md hover:bg-black transition-all">Usuń</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
        ...
        {{-- KLIENCI (PRZYWRÓCONO PRZYCISK USUŃ) --}}
                ...
                <table class="w-full text-left border-collapse">
                    ...
                    <tbody class="divide-y-2 divide-gray-100 text-sm">
                        ...
                        <tr class="hover:bg-green-50 transition-colors">
                            <td class="p-5">
                                <div class="font-black text-blue-900 uppercase tracking-tight">{{ $u->name }}</div>
                                <div class="text-[9px] text-gray-400 font-black mt-1 italic uppercase tracking-widest">SYST-ID: {{ $u->id }}</div>
                            </td>
                            <td class="p-5">
                                <div class="font-black text-gray-900">NIP: {{ $u->client->nip ?? '---' }}</div>
                                <div class="text-[10px] text-gray-500 uppercase font-bold tracking-tighter">{{ $u->client->address ?? 'Brak adresu' }}</div>
                            </td>
                            <td class="p-5">
                                <div class="flex justify-center gap-3">
                                    <a href="{{ route('admin.users.edit', $u) }}" aria-label="Edytuj klienta: {{ $u->name }}" class="bg-blue-800 text-white px-4 py-2 rounded font-black text-[10px] uppercase shadow-md hover:bg-black transition-all">Edycja</a>
                                    {{-- PRZYCISK USUŃ PRZYWRÓCONY --}}
                                    <form action="{{ route('admin.users.destroy', $u) }}" method="POST" onsubmit="return confirm('Czy na pewno chcesz trwale usunąć konto klienta {{ $u->name }}?')">
                                        @csrf @method('DELETE')
                                        <button aria-label="Usuń klienta: {{ $u->name }}" class="bg-red-700 text-white px-4 py-2 rounded font-black text-[10px] uppercase shadow-md hover:bg-black transition-all">Usuń</button>
                                    </form>
                                </div>
                            </td>
                        </tr>
